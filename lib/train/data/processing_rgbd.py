...
class RGBDProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        """
        data["template_depths"][0] = data["template_depths"][0] / 1000 * 299
        data["search_depths"][0] = data["search_depths"][0] / 1000 * 299
        ...
        t_cloud = pixel2point_cloud(self.output_sz["template"], self.output_sz["template"],
                                    color=torch.Tensor(data["template_images"]),
                                    depth=torch.Tensor(data["template_depths"]).unsqueeze(0)
                                    )
        _, t_bev_map_arr = point_cloud2BEV(self.output_sz["template"], self.output_sz["template"], max_depth=300,
                                           depth_map=data["template_depths"][0], cloud=t_cloud)

        _, t_siv_map_arr = point_cloud2SiV(self.output_sz["template"], self.output_sz["template"], max_depth=300,
                                           depth_map=data["template_depths"][0], cloud=t_cloud)

        s_cloud = pixel2point_cloud(self.output_sz["search"], self.output_sz["search"],
                                    color=torch.Tensor(data["search_images"]),
                                    depth=torch.Tensor(data["search_depths"]).unsqueeze(0)
                                    )
        _, s_bev_map_arr = point_cloud2BEV(self.output_sz["search"], self.output_sz["search"], max_depth=300,
                                           depth_map=data["search_depths"][0], cloud=s_cloud)

        _, s_siv_map_arr = point_cloud2SiV(self.output_sz["search"], self.output_sz["search"], max_depth=300,
                                           depth_map=data["search_depths"][0], cloud=s_cloud)

        data['search_bev'] = torch.Tensor(s_bev_map_arr).permute(2, 0, 1).unsqueeze(0)
        data['search_siv'] = torch.Tensor(s_siv_map_arr).permute(2, 0, 1).unsqueeze(0)
        data['template_bev'] = torch.Tensor(t_bev_map_arr).permute(2, 0, 1).unsqueeze(0)
        data['template_siv'] = torch.Tensor(t_siv_map_arr).permute(2, 0, 1).unsqueeze(0)
